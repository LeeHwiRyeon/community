...
const CreatePostPage: React.FC = () => {
    ...
    const [selectedBoardId, setSelectedBoardId] = useState<string>(safeBoardId)
    ...
    const mapFormToDraft = useCallback((values: CreatePostFormValues): DraftPayloadInput => {
        const attachmentsMetadata = values.attachments?.map(selectionToMetadata) ?? []
        return {
            title: values.title ?? "",
    ...
    // boards가 로딩된 후 selectedBoardId 유효성 확인
    useEffect(() => {
        if (boards.length > 0 && !boards.find(board => board.id === selectedBoardId)) {
        ...
        <div className="create-post-page">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="create-post-form">
                ...
                <div className="form-group">
                    <label htmlFor="board">게시판 선택</label>
                    <select
                        id="board"
                ...
                <div className="form-group">
                    ...
                    <AttachmentUploader
                        ...
                        onError={(error) => toast({ title: "Attachment upload failed.", description: error.message, status: "error", duration: 4000, isClosable: true })}
                    />
                </div>
